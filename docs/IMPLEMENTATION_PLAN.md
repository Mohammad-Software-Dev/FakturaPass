...
9. **FP-010 — review queue:** local derived task queue, ownership, concurrency controls and direct resolution routes implemented. See REVIEW_QUEUE.md. Delivery/rejection/master-data exceptions await their underlying features.
10. **FP-004 and operational readiness:** local snapshot creation and isolated restore verification implemented; see RECOVERY.md. Replace local identities with the selected production OIDC provider and complete production membership/scoped-key behavior, then verify hosting, backups and recovery. Database-backed membership enforcement and existing-team access controls are implemented; see TEAM_ACCESS.md. Production provider and deployment decisions remain in TBD.md.
...
The complete remaining delivery sequence and active increment are tracked in [COMPLETION_PLAN.md](COMPLETION_PLAN.md).
